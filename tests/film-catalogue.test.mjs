...
test('all original and batch IDs are preserved once, dated, and supported by the player', async()=>{
  assert.equal(catalogue.videos.length,240);
  assert.equal(new Set(catalogue.videos.map(v=>v.id)).size,240);
...
test('sections use actual dates across 2016/2017/2020 and include entire archive',()=>{
  ...
  assert.equal(groups.archive.length,240);
  assert.equal(groups.twenties.length+groups.late.length+groups.early.length,240);
